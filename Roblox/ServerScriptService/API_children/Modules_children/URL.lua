-- MODIFIED BY ROBLOX@gaberocksall ON DEC 16 2021
--     :normalize() will now add a trailing slash if necessary
--     added :copy()
--     made __div first make a copy
--     modified addSegment so that if you pass a table, it will loop through and individually add each segment
--     modified addSegment to first stringify the passed argument
--     added `normalize` argument to :build which defaults to true
...
local function concat(a, b)
    if type(a) == 'table' then
        return a:build(false) .. b
    else
        return a .. b:build(false)
    end
end
...
function M:build(normalize)
    if normalize ~= false then
        return self:copy():normalize():build(false)
    end
...
function M:normalize()
    ...
    if self.path then
        ...
        if not self.path:match("%.[^/]+$") and not self.path:match("/+$") then
            -- trailing slash as long as the path is not to a file
            -- and the path does not already end with a slash
            self.path ..= "/" 
        end
...
function M:copy()
    ...
    if self.query then
        ...
        setmetatable(cpy.query, getmetatable(self.query))
    end
    
    setmetatable(cpy, getmetatable(self))
